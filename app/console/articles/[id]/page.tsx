...
const TAGS = ["Risk", "Protocol", "Market", "Security", "Research", "Tutorial", "Ecosystem"];
...
export default function ArticleEditorPage() {
  ...
  useEffect(() => {
    if (isNew) return;
    fetch(`/api/articles?id=${encodeURIComponent(params.id)}&locale=pt`, { credentials: "include" })
      .then(async (r) => {
        if (!r.ok) return;
      ...
      <div className="mx-auto max-w-5xl px-6 pt-10 pb-20">
        ...
        <div className="flex items-center justify-between border-b border-white/6 pb-6">
          <div>
            <div className="flex items-center gap-2 text-[11px] text-neutral-500">
              ...
              <span>/</span>
              <Link href="/console/articles" className="hover:text-white transition-colors">Articles</Link>
              <span>/</span>
              <span className="text-neutral-300">{isNew ? "New" : "Edit"}</span>
            </div>
            <h1 className="mt-1 text-[24px] font-semibold tracking-tight">{isNew ? "New article" : "Edit article"}</h1>
          </div>
        ...
        <div className="mt-6 space-y-4">
          ...
          <div>
            ...
            {preview ? (
              <div className="min-h-[400px] w-full rounded-xl border border-white/[0.07] bg-white/2 p-5 text-[14px] leading-7 text-neutral-300 whitespace-pre-wrap font-mono text-[12px]">
