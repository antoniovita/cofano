  ...
  while (i < lines.length) {
    ...
    if (ulMatch) {
      ...
      nodes.push(
        <ul
          key={`ul-${key++}`}
          className="my-5 ml-5 list-disc space-y-2 text-[15px] leading-7 text-neutral-200"
        >
          {items.map((it, itIdx) => (
    ...
    if (olMatch) {
      ...
      nodes.push(
        <ol
          key={`ol-${key++}`}
          className="my-5 ml-5 list-decimal space-y-2 text-[15px] leading-7 text-neutral-200"
        >
          {items.map((it, itIdx) => (
...
function toViewArticleFromApi(article: ApiArticle): ViewArticle {
  const readMinutes = computeReadMinutes(article);
  return {
    id: article.id,
    title: article.title,
    ...
    return (
      <main className="flex-1 bg-[#0f0f0f] text-white">
        <section className="mx-auto max-w-5xl px-6 py-10">
          <Link href="/articles" className="text-[14px] text-neutral-400 hover:text-neutral-200 transition-colors">
            ← Back to articles
          </Link>
          {status === "loading" ? (
    ...
    <main className="flex-1 bg-[#0f0f0f] text-white">
      <section className="mx-auto max-w-5xl px-6 py-10">
        <div className="flex items-center justify-between gap-4">
          <Link
            href="/articles"
            className="text-[14px] text-neutral-400 hover:text-neutral-200 transition-colors"
          >
            ← Back to articles
          </Link>
          <ShareButton
